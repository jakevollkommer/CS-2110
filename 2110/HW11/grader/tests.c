...
void check_malloc_2b() {
    char* tag = "check_malloc_2b - smallest fit";

    int size1 = 2*WORD_SIZE;
    void* v1 = my_malloc(size1);

    int size2 = SBRK_SIZE - TOTAL_METADATA_SIZE*WORD_SIZE*3 - 5*WORD_SIZE;
    my_malloc(size2);
    my_free(v1);

    // freelist -> {size=6} -> {size=7} on 64-bit
    // freelist -> {size=5} -> {size=6} on 32-bit

    int size3 = WORD_SIZE;
    my_malloc(size3);

    if (total_freelist_block_size() != TOTAL_METADATA_SIZE+3 || total_freelist_count() != 1 || !is_freelist_in_order()) {
        deduct_points(M_EXACT_FIT_POINTS, tag, "Did not select smallest fit from the freelist;");
    } else {
        add_points(M_EXACT_FIT_POINTS, tag, "Selected exact next smallest freelist;");
    }
}
